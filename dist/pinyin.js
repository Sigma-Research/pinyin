...
var dict = {
    ...
    "qi": "奇期欺栖戚妻七凄漆柒沏其棋奇歧畦崎脐齐旗祈祁骑起岂乞企启契砌器气迄弃汽泣讫亟亓圻芑萋葺嘁屺岐汔淇骐绮琪琦杞桤槭欹祺憩碛蛴蜞綦綮趿蹊鳍麒",
    "qia": "掐恰洽葜",
...
var convertFull = (function (text) {
    ...
    return { full: full, short: short, map: map, text: text, toString: toString };
});
 ...
 */
var exec = (function (pinyin, filter) {
...
var stringToPath = function stringToPath(string) {
    var result = [];
    string.replace(RE_PROP_NAME, function (match, number, quote, string) {
...
var getValueByPath = function getValueByPath(obj, path) {
    ...
    for (var len = keys.length; i < len - 1; i++) {
        if (!obj && !strict) break;
        var key = keys[i];
        if ((typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object' && key in obj) {
            obj = obj[key];
        } else {
            if (strict) {
 ...
 */
var init = (function (input, fields) {
    var pinyinKey = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : '$$pinyin';

    if (!input) return;
    var setAttr = function setAttr(item) {
        var pinyinData = getValueByPath(item, pinyinKey);
        if (!pinyinData) setValueByPath(item, pinyinKey, pinyinData = {});
        if (!fields) {
            fields = [];
            for (var field in pinyinData) {
                fields.push(field);
            }
        }
 ...
 * @returns {*} 满足结果的项
 */
var search = (function (input, filter, fields) {
    var or = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
    ...
    var test = function test(item) {
        // 某字段是否已经匹配到过关键词
        var checked = {};
        var testField = function testField(filter, field) {
            var pinyin = getValueByPath(getValueByPath(item, pinyinKey), field);
            var match = exec(pinyin, filter);
...
exports.convertFull = convertFull;
exports.exec = exec;
exports.test = test;
